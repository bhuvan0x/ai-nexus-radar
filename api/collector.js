  ...
  try{
    if(req.method==='POST'){
      ...
      try{
        ...
        if(e.status===400&&QUOTA_HINT.test(msg))return res.status(402).json({error:'Your Bright Data trial has no custom collector slots left.',code:'CUSTOM_COLLECTOR_QUOTA',detail:msg,action:'Reuse an existing collector or ask Bright Data/hackathon support to increase your collector allowance.'});
        throw e;
      }
    ...
    if(req.method==='GET'){
      const id=String(req.query?.collectorId||process.env.COLLECTOR_ID||'');
