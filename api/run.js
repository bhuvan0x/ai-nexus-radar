...
function isBatchUnsupported(error) {
  ...
  return /cannot trigger a batch job with a real-time scraper|batch.*not.*support|unsupported.*batch|invalid.*batch|unknown.*endpoint|method.*not.*allowed/.test(message) || ['batch_not_supported', 'unsupported_batch'].includes(code);
}
...
async function triggerBatch(collectorId, url) {
  ...
  if (!collectionId) throw new Error(`Bright Data accepted the batch request but returned an unexpected payload: ${JSON.stringify(result)}`);
  return { responseId: `batch:${collectionId}`, collectionId, status: 'pending', mode: 'batch', collectorId, url };
}
  ...
  try {
    ...
    if (!Array.isArray(result)) {
      ...
      if (['failed', 'error', 'cancelled', 'canceled'].includes(status)) {
        return { status: 'error', mode: 'batch', snapshotId, error: stringifyError(result?.error || result), error_code: result?.error_code || 'BATCH_JOB_FAILED' };
      }
    ...
    if (scrapeError) return { status: 'error', mode: 'batch', snapshotId, error: scrapeError.message, error_code: scrapeError.error_code, details: scrapeError.failures };
    return { status: 'done', mode: 'batch', snapshotId, data: result, rowCount: result.length, rawType: 'array' };
  } catch (error) {
    // A just-created snapshot can briefly be unavailable. Surface 404 as a
    // bounded transient state; do not retry forever in the frontend.
    if (error.status === 404 || error.status === 425 || error.status === 429 || error.status === 503) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      try {
        ...
        const realtime = await request(`/dca/trigger_immediate?collector=${encodeURIComponent(collectorId)}`, { method: 'POST', body: JSON.stringify({ url }) });
    ...
    if (req.method === 'GET') {
      ...
      if (token.startsWith('batch:')) {
        ...
        if (snapshot.status === 'error') return res.status(502).json({ error: snapshot.error, error_code: snapshot.error_code, details: snapshot.details, code: 'BRIGHTDATA_RESULT_ERROR' });
        return res.status(200).json(snapshot);
      }

      try {
        const result = await request(`/dca/get_result?response_id=${encodeURIComponent(token)}`);
        if (result?.pending === true || /pending|running|processing|queued/i.test(String(result?.status || ''))) return res.status(200).json({ status: 'pending', mode: 'realtime' });
        const data = rows(result);
        const scrapeError = extractResultError(data);
        if (scrapeError) return res.status(502).json({ error: scrapeError.message, error_code: scrapeError.error_code, details: scrapeError.failures, code: 'BRIGHTDATA_RESULT_ERROR' });
        ...
        if (error.status === 404) return res.status(404).json({ status: 'missing', mode: 'realtime', error: 'Bright Data no longer has this response ID. The job may have expired or been removed.', code: 'RESPONSE_NOT_FOUND' });
        throw error;
      }
    }
