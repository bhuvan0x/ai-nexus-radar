...
async function request(path,options={}){
  ...
  if(!res.ok){const msg=data?.error||data?.message||data?.raw||`Bright Data HTTP ${res.status}`;const err=new Error(`${String(msg)} [HTTP ${res.status}]`);err.status=res.status;err.details=data;throw err;}
  return data;
}
